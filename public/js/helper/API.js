...
async function getAllDataSumberInformasi(url) {
  try {
    ...
    Swal.fire({
      icon: "error",
      title: "Gagal",
      text: err.responseJSON?.message || err.statusText || "Terjadi kesalahan saat mengambil data.",
    });
    throw err;
  }
}
...
async function fetchDataSumberInformasiDetail(url) {
    try {
        const response = await fetch(`${url}/bukutamu/allDataSumberInformasiDetail`);
        const result = await response.json();
